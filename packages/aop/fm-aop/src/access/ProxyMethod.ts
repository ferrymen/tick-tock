...
import { IPointcut, Joinpoint, JoinpointState } from '../joinpoints';
import { ProxyMethodToken, IProxyMethod } from './IProxyMethod';
...
export class ProxyMethod implements IProxyMethod {
  ...
  proceed(
    ...
    if (advices && pointcut) {
      if (
        pointcut.descriptor &&
        (pointcut.descriptor.get || pointcut.descriptor.set)
      ) {
        if (pointcut.descriptor.get) {
          let getMethod = pointcut.descriptor.get.bind(target);
          pointcut.descriptor.get = this.proxy(
            getMethod,
            advices,
            target,
            targetType,
        ...
        if (pointcut.descriptor.set) {
          ...
          pointcut.descriptor.set = this.proxy(
            setMethod,
            advices,
            target,
            targetType,
  ...
  proxy(
    ...
    return (...args: any[]) => {
      let joinPoint = this.container.resolve(
        Joinpoint,
        Provider.create('options', {
          name: methodName,
          fullName: fullName,
          provJoinpoint: provJoinpoint,
          annotations: provJoinpoint
            ? null
            : liefScope.getMethodMetadatas(targetType, methodName),
      ...
      let adChain = container.resolve(AdvisorChainFactoryToken, {
        container: container,
