...
export class AdviceMatcher implements IAdviceMatcher {
  ...
  match(
    ...
    let aspectMeta = Lang.first(
      getOwnTypeMetadata<AspectMetadata>(Aspect, aspectType)
    );
    if (aspectMeta) {
      if (aspectMeta.within) {
        let ins = isArray(aspectMeta.within)
          ? aspectMeta.within
          : [aspectMeta.within];
      ...
      if (aspectMeta.annotation) {
        let annotation = isFunction(aspectMeta.annotation)
          ? aspectMeta.annotation.toString()
          : aspectMeta.annotation;
    ...
    adviceMetas =
      adviceMetas || getOwnMethodMetadata<AdviceMetadata>(Advice, targetType);
    ...
    if (targetType === aspectType) {
      ...
      // if (!advisor.hasRegisterAdvices(targetType)) {
      ...
      Object.getOwnPropertyNames(adviceMetas).forEach(name => {
        ...
        advices.forEach(metadata => {
          matched = matched.concat(
            this.filterPointcut(targetType, points, metadata)
          );
        });
      });
    }
  ...
  filterPointcut(
    ...
    if (metadata.pointcut) {
      ...
      matchedPointcut = points.filter(p =>
        match(p.name, p.fullName, type, target, p)
      );
    }
  ...
  protected matchTypeFactory(
    ...
    if (metadata.within) {
      expresses.push(
        (method: string, fullName: string, targetType?: Type<any>) => {
    ...
    if (isString(pointcut)) {
      ...
      if (/^\^?@\w+/.test(pointcutReg.source)) {
        expresses.push(
          (name: string, fullName: string, targetType?: Type<any>) => {
            let decName = Reflect.getMetadataKeys(type, name);
            return decName.some(n => isString(n) && pointcutReg.test(n));
          }
        );
      } else {
        expresses.push((name: string, fullName: string) =>
          pointcutReg.test(fullName)
        );
      }
    }
  ...
  protected expressToFunc(type: Type<any>, strExp: string): MatchExpress {
    if (/^@annotation\(.*\)$/.test(strExp)) {
      ...
      return (name: string, fullName: string) =>
        hasOwnMethodMetadata(annotation, type, name) &&
        !hasOwnClassMetadata(Aspect, type);
    } else if (/^execution\(.*\)$/.test(strExp)) {
      let exp = strExp.substring(10, strExp.length - 1);
      if (exp === '*' || exp === '*.*') {
        return (name: string, fullName: string) =>
          !!name && !hasOwnClassMetadata(Aspect, type);
      ...
      let classnames = strExp
        .substring(strExp.indexOf('(') + 1, strExp.length - 1)
        .split(',')
        .map(n => n.trim());
      return (name: string, fullName: string, targetType?: Type<any>) =>
        classnames.indexOf(getClassName(targetType)) >= 0;
    } else if (/^@target\(\s*\w+/.test(strExp)) {
      let torken = strExp
        .substring(strExp.indexOf('(') + 1, strExp.length - 1)
        .trim();
      return (name: string, fullName: string, targetType?: Type<any>) =>
        this.container.getTokenImpl(torken) === targetType;
    } else {
      return () => false;
    }
  }
  ...
  protected mergeExpress(
    ...expresses: (MatchExpress | string)[]
  ): MatchExpress {
    return (
      method: string,
      fullName: string,
