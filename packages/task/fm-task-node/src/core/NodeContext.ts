import { Singleton, ObjectMap, Express2 } from '@ferrymen/fm-ioc-core';

import { normalize, join, dirname } from 'path';

import { readdirSync, lstatSync, existsSync } from 'fs';

import { Src, Context } from '@ferrymen/fm-task-core';
import { toAbsolutePath } from '@ferrymen/fm-ioc-platform-server';
import { NodeContextToken, INodeContext, CmdOptions } from './INodeContext';
...
export class NodeContext extends Context implements INodeContext {
  ...
  hasArg(arg): boolean {
    return (
      process.argv.indexOf(arg) > -1 || process.argv.indexOf('--' + arg) > -1
    );
  }
