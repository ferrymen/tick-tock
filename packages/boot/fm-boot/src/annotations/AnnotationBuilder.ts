...
export class AnnotationBuilder<T> implements IAnnotationBuilder<T> {
  ...
  async build(
    ...
    if (!this.isEqual(builder)) {
      ...
      instance = (await this.buildStrategy(
        instance,
        config,
        data
      )) as AnnoInstance<T>;
  ...
  async createInstance(
    ...
    if (!this.container.has(token)) {
      console.log(
        `can not find token ${token ? token.toString() : null} in container.`
      );
      return null;
    }
  ...
  getBuilder(
    ...
    let providers = [
      { provide: ContainerToken, useValue: this.container },
      { provide: Container, useValue: this.container },
    ] as Providers[];
    if (config && config.annotationBuilder) {
      ...
      if (isToken(config.annotationBuilder)) {
        builder = this.container.resolve(
          config.annotationBuilder,
          ...providers
        );
  ...
  protected getTokenMetaConfig(
    token: Token<T>,
    config?: AnnotationConfigure<T>
  ): AnnotationConfigure<T> {
    ...
    if (isClass(token)) {
      ...
      let tokenType = this.container
        ? this.container.getTokenImpl(token)
        : token;
