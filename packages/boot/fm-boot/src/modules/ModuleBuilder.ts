...
import { ContainerPoolToken, ContainerPool } from '../utils';
...
export class ModuleBuilder<T> implements IModuleBuilder<T> {
  ...
  async build(
    ...
    let annBuilder = this.getAnnoBuilder(
      container,
      injmdl.token,
      cfg.annotationBuilder
    );
    if (!injmdl.token) {
  ...
  async bootstrap(
    ...
    let anBuilder = this.getAnnoBuilder(
      container,
      bootToken,
      cfg.annotationBuilder
    );
    let bootInstance = await (bootToken
      ? anBuilder.build(bootToken, cfg, data)
      : anBuilder.buildByConfig(cfg, data, this.getType(cfg)));
    let runable;
    if (bootInstance) {
      runable = await this.autoRun(
        container,
        bootToken ? bootToken : anBuilder.getType(cfg),
        cfg,
        bootInstance,
        data
      );
  ...
  protected async load(
    ...
    if (isToken(token)) {
      ...
      if (!injmdl) {
        let cfg = parent
          .get(AnnotationMetaAccessorToken)
          .getMetadata(token, parent);
  ...
  protected async autoRun(
    ...
    if (instance instanceof Runner) {
      ...
      let providers = [
        ...
        { token: token, instance: instance, config: cfg },
      ] as Providers[];
  ...
  protected getAnnoBuilder(
    container: IContainer,
    token: Token<any>,
    annBuilder: Token<IAnnotationBuilder<any>> | IAnnotationBuilder<any>
  ): IAnyTypeBuilder {
  ...
  protected getDefaultAnnBuilder(
    container: IContainer
  ): IAnnotationBuilder<any> {
