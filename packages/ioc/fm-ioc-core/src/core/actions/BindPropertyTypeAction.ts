 ...
 *
 */
export interface BindPropertyTypeActionData
...
export class BindPropertyTypeAction extends ActionComposite {
  ...
  protected working(container: IContainer, data: BindPropertyTypeActionData) {
    ...
    let matchs = lifeScope.getPropertyDecorators(
      surm =>
        surm.actions.includes(CoreActions.bindPropertyType) &&
        hasPropertyMetadata(surm.name, type)
    );
