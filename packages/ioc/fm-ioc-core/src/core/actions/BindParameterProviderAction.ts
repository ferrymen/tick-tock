 ...
 *
 */
export interface BindParameterProviderActionData
...
export class BindParameterProviderAction extends ActionComposite {
  ...
  protected working(
    ...
    data: BindParameterProviderActionData
  ) {
    ...
    let matchs = lifeScope.getMethodDecorators(
      surm =>
        surm.actions.includes(CoreActions.bindParameterProviders) &&
        hasOwnMethodMetadata(surm.name, type)
    );
