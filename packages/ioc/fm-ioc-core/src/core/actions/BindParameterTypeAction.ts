...
export class BindParameterTypeAction extends ActionComposite {
  ...
  protected working(container: IContainer, data: ActionData<Token<any>[]>) {
    ...
    let matchs = lifeScope.getParameterDecorators(surm => {
      return (
        surm.actions.includes(CoreActions.bindParameterType) &&
        (target || propertyKey !== 'constructor'
          ? hasParamMetadata(surm.name, target, propertyKey)
    ...
    matchs.forEach(surm => {
      let parameters =
        target || propertyKey !== 'constructor'
          ? getParamMetadata<ParameterMetadata>(surm.name, target, propertyKey)
          : getOwnParamMetadata<ParameterMetadata>(surm.name, type);
          ...
          if (parm && parm.index >= 0) {
            if (lifeScope.isVaildDependence(parm.provider)) {
              if (!container.has(parm.provider, parm.alias)) {
                container.register(
                  container.getToken(parm.provider, parm.alias)
                );
              }
            }
            ...
            let token = parm.provider
              ? container.getTokenKey(parm.provider, parm.alias)
              : parm.type;
            if (token) {
